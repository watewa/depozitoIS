<nav x-data="{ open: false }" class="bg-white  border-b border-gray-100 ">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
                    ...
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500  bg-white  hover:text-gray-700  focus:outline-none transition ease-in-out duration-150">
                            <div class="flex-shrink-0 h-30 w-30">
                                <img style="height: 30px; width: 30px; object-fit: cover; border-radius: 50%;" src="{{ Auth::user()->getProfilePicturePathAttribute() }}" alt="{{ 'Profile Picture' }}">
                            </div>
                            <div class="ml-3">
                                {{ Auth::user()->name }}
                            </div>
            ...
            @else
            <div class="hidden sm:flex sm:items-center justify-content-right">
                <div class="hidden space-x-8 sm:-my-px sm:flex mr-1 h-100">
                    <x-nav-link :href="route('login')" :active="request()->routeIs('login')">
                        {{ __('Prisijungti') }}
                    </x-nav-link>
                </div>
                <div class="hidden space-x-8 sm:-my-px sm:flex ml-1 h-100">
